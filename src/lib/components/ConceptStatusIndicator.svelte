<script lang="ts">
  ...
  export let conceptName: string | undefined = undefined;
  ...
  onMount(() => {
    unsubscribe = concepts.subscribe((storeValue) => {
      ...
      // Update individual indicator if conceptName is provided
      if (conceptName && conceptsSnapshot[conceptName]) {
        value = conceptsSnapshot[conceptName].value;
        isActive = conceptsSnapshot[conceptName].isActive;
      }
    });
  });
...
<style>
  ...
  /* Global concept status indicator styles */
